...
def IK(vec):
    ...
    Args: 6*1 end-effector Pose, position + orientation in euler in degrees
    """
    no_sol = False
    (px, py, pz, rx, ry, rz) = (vec[0], vec[1], vec[2]+0.33, vec[3]/180*math.pi, vec[4]/180*math.pi, vec[5]/180*math.pi)
    ...
    # DH-parameters (with an additional J6->2 Gripper)
    d = np.array([0.3300, 0.0000, 0.0000, -0.4200, 0.0000, -0.0800-0.1447]) 
...
def FK(q_vec):
    ...
    return [x,y,z] of end-effector + Orientation in degrees
    """
    dof = 6
    # DH-parameters
    d = np.array([0.3300, 0.0000, 0.0000, -0.4200, 0.0000, -0.0800-0.1447])        
    ...
    # Convert to the robot frame, which is located at the center of joint 2
    sol[2] -= 0.33
    return sol
